...
To fully remove the bridge: quit it from the tray menu (or stop the process if run from a terminal), then uninstall the application the normal way for your OS (drag the `.app` to Trash on macOS, or use "Apps & Features" / the uninstaller on Windows). No account, cloud service, or system-level daemon is registered, so uninstalling the application is the entire removal.
